<!--
  AppIcons.vue - 通用 SVG 图标（纯 inline）
  使用：<app-icon name="shield" /> 或 :size="24" :color="'#fff'"
-->
<template>
  <span class="app-icon" :style="{ width: size, height: size, color }" v-html="svg" />
</template>
...
// 用 heroicons 风格的 24x24 stroke 矢量图标（精简版）
const paths: Record<string, string> = {
  // 盾牌-安全（loading 用）
  shield: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2L4 5v6.09c0 5.05 3.41 9.76 8 10.91 4.59-1.15 8-5.86 8-10.91V5l-8-3z"/><path d="M9 12l2 2 4-4"/></svg>`,
  // 铃铛（提示用）
  ...
  // 身份证
  idcard: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="5" width="20" height="14" rx="2"/><circle cx="8" cy="12" r="2"/><path d="M14 10h4M14 14h4M8 16h6"/></svg>`,
  // 手机
  phone: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="6" y="2" width="12" height="20" rx="2"/><line x1="11" y1="18" x2="13" y2="18"/></svg>`,
  // 锁
